...
-- local weppouch = CreateClientConVar("arcticvr_weppouch", "0", FCVAR_ARCHIVE)
function SWEP:VRInput(action, state)
    ...
    --     if LocalPlayer():LookupBone(weppouchbone) and LocalPlayer():GetBoneMatrix(LocalPlayer():LookupBone(weppouchbone)) then
    --         weppouchdist = LocalToWorld(Vector(3, 3, 0), Angle(0, 0, 0), LocalPlayer():GetBoneMatrix(LocalPlayer():LookupBone(weppouchbone)):GetTranslation(), Angle(0, g_VR.characterYaw, 0))
    ...
    --             elseif action == "boolean_right_pickup" and not state then
    --                 if g_VR.tracking.pose_righthand.pos:DistToSqr(weppouchdist) < (weppouchsize * weppouchsize) then
